...
class TestSmartSystem extends System<ITestContext> {
  public setup() {
    return {
      ...
      changed: this.world.createEventChannel(
        ComponentEvent.Changed,
        TestComponent
      ),
      entities: this.world.createQuery(
        Write(TestComponent),
        Write(ObserverComponent)
      ),
      removed: this.world.createEventChannel(
  ...
  public update() {
    ...
    for (const [entityId, testComponent] of this.ctx.added) {
      const observerComponent = this.world.getMutableComponent(
        entityId,
        ObserverComponent
      );
